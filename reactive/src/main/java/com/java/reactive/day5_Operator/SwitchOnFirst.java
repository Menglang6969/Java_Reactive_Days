...
public class SwitchOnFirst {
    public static void main(String[] args) {
        getStudent()
                .switchOnFirst((signal,flux)->{
                   if(signal.isOnNext()&& Objects.requireNonNull(signal.get()).getAge()>15){
                       return mapStudent().apply(flux);
                   }
                    return flux;
                })
    ...
    private static Function<Flux<Student>,Flux<Student>> mapStudent(){
        return flux->flux.filter(x->x.getAge()>15)
                ...
                .doOnDiscard(Student.class,x->System.out.println("Student Not Allow: "+x.getName()+" : "+x.getAge()));
    }

    private static Flux<Student> getStudent() {
